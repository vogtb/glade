...
export function LiveDemo() {
  const currentDemoName = useStore(selectedDemo);
  const [activeTab, setActiveTab] = useState<"demo" | "code">("demo");

  const currentDemo =
    componentDemos.find((demo) => demo.name === currentDemoName) ?? componentDemos[0];
      ...
      >
        ...
        <div className="pt-1 grid grid-cols-3 items-center w-full">
          ...
          <div className="hidden md:flex justify-center px-3 py-1">
            <span className="font-mono font-medium text-[15px] text-white">{currentDemoName}</span>
          </div>
