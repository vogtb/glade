...
type TableCellRequestState = ReturnType<GladeDiv["requestLayout"]>["requestState"];
type TableCellPrepaintState = ReturnType<GladeDiv["prepaint"]>;

/**
 * Root table element. Owns the grid and aligns all cells.
 */
export class GladeTable extends GladeContainerElement<TableRequestState, TablePrepaintState> {
  private styles: Partial<Styles> = { display: "grid" };
  private columnTemplateValue: GridTemplate | null = null;
  private cellsForHitTest: GladeTableCell[] = [];
  ...
  private collectSections(): GladeTableSection[] {
    ...
    for (const child of this.getChildren()) {
      if (child instanceof GladeTableSection) {
        ...
        const body = new GladeTableSection("body");
        body.child(child);
        sections.push(body);
      }
    }
  ...
  private collectRows(section: GladeTableSection): GladeTableRow[] {
    const rows: GladeTableRow[] = [];
    const candidates = section.getChildren();
    for (const candidate of candidates) {
      if (candidate instanceof GladeTableRow) {
        rows.push(candidate);
      }
    }
    return rows;
  ...
  private collectCells(row: GladeTableRow): GladeTableCell[] {
    const cells: GladeTableCell[] = [];
    const candidates = row.getChildren();
    for (const candidate of candidates) {
      if (candidate instanceof GladeTableCell) {
 ...
 */
export class GladeTableSection extends GladeContainerElement<null, null> {
  ...
  requestLayout(): RequestLayoutResult<null> {
    throw new Error("GladeTableSection must be used inside table().");
  }
 ...
 */
export class GladeTableRow extends GladeContainerElement<null, null> {
  requestLayout(): RequestLayoutResult<null> {
    throw new Error("GladeTableRow must be used inside table().");
  }
...
/**
 * Table cell. Renders via GladeDiv and stores span metadata for table layout.
 */
export class GladeTableCell extends GladeDiv {
...
export function table(): GladeTable {
  return new GladeTable();
}

export function thead(): GladeTableSection {
  return new GladeTableSection("head");
}

export function tbody(): GladeTableSection {
  return new GladeTableSection("body");
}

export function tfoot(): GladeTableSection {
  return new GladeTableSection("foot");
}

export function tr(): GladeTableRow {
  return new GladeTableRow();
}

export function th(): GladeTableCell {
  return new GladeTableCell(true);
}

export function td(): GladeTableCell {
  return new GladeTableCell(false);
}
