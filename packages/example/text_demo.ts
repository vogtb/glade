...
type TextDemoItem = FlashElement<unknown, unknown>;

const textDemoListState = createListState();
let textDemoScrollHandle: ScrollHandle | null = null;

export const TEXT_DEMO: Demo = {
  name: "Text",
  renderElement: (cx) => {
    const scrollHandle = textDemoScrollHandle ?? cx.newScrollHandle(cx.windowId);
    textDemoScrollHandle = scrollHandle;

    const items: TextDemoItem[] = [
    ...
    return div()
      ...
      .flex1()
      .child(
        list<TextDemoItem>((item, _props, _cx) => item, textDemoListState)
          .data(items)
          .estimatedItemHeight(36)
          .setOverdraw(3)
          .trackScroll(scrollHandle)
          .setContext(cx)
          .scrollbarAlways()
          .flex1()
      );
