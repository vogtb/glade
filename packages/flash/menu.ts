...
function getMenuItemKind(item: MenuItemElement): DropdownMenuItemKind {
  if (item instanceof FlashDropdownItem) return "item";
  if (item instanceof FlashDropdownCheckbox) return "checkbox";
  if (item instanceof FlashDropdownRadio) return "radio";
  if (item instanceof FlashDropdownSub) return "sub";
  if (item instanceof FlashDropdownSeparator) return "separator";
  return "label";
}
...
function prepaintMenuItem(
  ...
  switch (kind) {
    case "item":
      if (item instanceof FlashDropdownItem && isDropdownItemRequestState(requestState)) {
        return { kind, state: item.prepaint(childCx, bounds, requestState) };
      }
      break;
    case "checkbox":
      if (item instanceof FlashDropdownCheckbox && isDropdownCheckboxRequestState(requestState)) {
        return { kind, state: item.prepaint(childCx, bounds, requestState) };
      }
      break;
    case "radio":
      if (item instanceof FlashDropdownRadio && isDropdownRadioRequestState(requestState)) {
        return { kind, state: item.prepaint(childCx, bounds, requestState) };
      }
      break;
    case "sub":
      if (item instanceof FlashDropdownSub && isDropdownSubRequestState(requestState)) {
        return { kind, state: item.prepaint(childCx, bounds, requestState) };
      }
      break;
    case "separator":
      if (item instanceof FlashDropdownSeparator && isDropdownSeparatorRequestState(requestState)) {
        return { kind, state: item.prepaint(childCx, bounds, requestState) };
      }
      break;
    case "label":
      if (item instanceof FlashDropdownLabel && isDropdownLabelRequestState(requestState)) {
        return { kind, state: item.prepaint(childCx, bounds, requestState) };
      }
      break;
  }
...
function paintMenuItem(
  ...
  switch (entry.kind) {
    case "item":
      ...
      break;
    case "checkbox":
      if (item instanceof FlashDropdownCheckbox) {
        item.paint(childCx, bounds, entry.state);
      }
      break;
    case "radio":
      ...
      break;
    case "sub":
      if (item instanceof FlashDropdownSub) {
        item.paint(childCx, bounds, entry.state);
      }
      break;
    case "separator":
      if (item instanceof FlashDropdownSeparator) {
        item.paint(childCx, bounds, entry.state);
      }
      break;
    case "label":
      if (item instanceof FlashDropdownLabel) {
        item.paint(childCx, bounds, entry.state);
      }
      break;
  }
