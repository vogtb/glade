 ...
 */
class FlashTabTrigger extends FlashElement<TabTriggerRequestState, TabTriggerPrepaintState> {
 ...
 */
export class FlashTab {
  private tabValue: string;
  private labelText: string;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private contentElement: FlashElement<any, any> | null = null;
  private disabledValue: boolean = false;
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  content(element: FlashElement<any, any>): this {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getContent(): FlashElement<any, any> | null {
   ...
   * @internal
   */
  createTrigger(): FlashTabTrigger {
    const trigger = new FlashTabTrigger(this.tabValue, this.labelText);
 ...
 */
export class FlashTabs extends FlashContainerElement<TabsRequestState, TabsPrepaintState> {
   ...
   */
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  private getActiveContent(): FlashElement<any, any> | null {
  ...
  prepaint(cx: PrepaintContext, bounds: Bounds, requestState: TabsRequestState): TabsPrepaintState {
    ...
    if (activeContent && contentElementId && requestState.contentLayoutId) {
      ...
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      contentPrepaintState = (activeContent as FlashElement<any, any>).prepaint(
        contentCx,
        contentBounds,
        contentRequestState
      );
  ...
  paint(cx: PaintContext, bounds: Bounds, prepaintState: TabsPrepaintState): void {
    ...
    if (activeContent && contentElementId && contentBounds) {
      const contentCx = cx.withElementId(contentElementId);
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      (activeContent as FlashElement<any, any>).paint(
        contentCx,
        contentBounds,
        contentPrepaintState
      );
 ...
 *   )
 */
export function tabs(): FlashTabs {
  return new FlashTabs();
}
 ...
 */
export function tab(value: string, label: string): FlashTab {
  return new FlashTab(value, label);
}
