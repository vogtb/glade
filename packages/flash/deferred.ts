...
export interface DeferredDrawEntry {
  child: FlashElement<unknown, unknown>;
  bounds: Bounds;
  offset: Point;
  priority: number;
  childElementId: GlobalElementId;
 ...
 */
export class DeferredElement extends FlashElement<
  DeferredRequestLayoutState,
  DeferredPrepaintState
> {
  private priorityValue = 0;
  private child: FlashElement<unknown, unknown> | null = null;

  constructor(child: FlashElement<unknown, unknown>) {
    super();
    this.child = child;
  }
 ...
 */
export function deferred(child: FlashElement<unknown, unknown>): DeferredElement {
