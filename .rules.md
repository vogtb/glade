...
- **Darwin (macOS)**: Leverages GLFW for window creation and event handling, with Dawn as the WebGPU implementation
...
The Darwin platform uses vendored C++ dependencies included as git submodules in `./vendor`:
...
- FFI bindings to native libraries (Darwin platform)
